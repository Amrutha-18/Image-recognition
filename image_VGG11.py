...
Original file is located at
    https://colab.research.google.com/drive/18CgQ0lXQ8WPllqYQTd__ORafQYU_ElR2
"""
...
class VGG11(nn.Module):
    def __init__(self, num_classes=36):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=2, stride=2),
            # No pooling here, to retain size 7x7
...
classification_model = VGG11(num_classes=36)
...
def get_predictions(model, test_loader):
    ...
    with torch.no_grad():
      for images, labels in test_loader:
          ...
          probabillity = torch.nn.functional.softmax(outputs, dim=1)
          _, predicted = torch.max(outputs.data, 1)
          predictions = predictions + predicted.cpu().numpy().tolist()
          true_labels = true_labels + labels.cpu().numpy().tolist()
          probabilities = probabilities + probabillity.cpu().numpy().tolist()

    return predictions, true_labels, probabilities

predictions, true_labels, probabilities = get_predictions(classification_model, test_loader)
len(predictions)
